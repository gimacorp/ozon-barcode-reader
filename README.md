...
Решение варианта 2 тестового задания по Computer Vision для мастерской Ozon. Автор: **Карим Гимадиев**. Все пояснения и отчёт подготовлены на русском языке.

**[Открыть PDF-отчёт, 21 страница](output/pdf/ozon_cv2_report_ru.pdf)** · [Текст отчёта](docs/report.md) · [Ноутбук](notebooks/demo_ru.ipynb) · [Исходные результаты](results/benchmark.json)
...
- Эксперимент на 60 синтетических коробках: 180 изображений, 140 эталонных кодов, три режима обработки.
...
Результат `decode` - JSON со всеми найденными уникальными значениями, символикой, исходными байтами и координатами четырёхугольника. Параметр `--enhanced` добавляет CLAHE и увеличение ×2. Ключ объединения: **символика + исходные байты**. Несколько наклеек с одинаковым содержимым представлены одним значением.
...
tests/                30 автоматических проверок
...
Первоисточники приведены в отчёте. Приоритет следующего практического этапа: проверить минимальный код на торцах при F/11 и 40 мкс, затем подтвердить механику нижнего обзора.
