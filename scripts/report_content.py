...
def make_pages(calc, bench, native, *, bundle=None):
    ...
    visual = bundle.get("visual_evidence") or read("results/visual_evidence.json")
    optical_result = ", ".join(visual["optical"]["found"]) or "пустой результат"
    real_count = sum(len(r["found"]) for r in visual["real"])
    false_caption = (
        f"Ложное {visual['false_read']['decoded']} в SYN-0023; формат {visual['false_read']['format']}"
        if visual["false_read"]["found"]
        else "Исторический пример SYN-0023; в текущем запуске ошибка не воспроизвелась"
    )
    pages = []
    p = lambda s: ("p", s)
    h = lambda s: ("h2", s)
    ...
    page(
        ...
        fig(
            "optical_code.png",
            f"Рис. 3. Моделирование минимального кода: результат после искажений — {optical_result}.",
        ),
        sm(
    ...
    page(
        ...
        fig(
            "false_reading.png",
            f"Рис. 9. {false_caption}. Эталон фрагмента — OZ002303.",
        ),
        p(
        ...
        fig(
            "real_sources.png",
            f"Рис. 10. Иллюстрации условия: расширенный 1D-поиск с обработкой дал {real_count} чтений на {len(visual['real'])} изображениях.",
        ),
        sm(
