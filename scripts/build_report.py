...
def build():
    ...
    class ReportDoc(SimpleDocTemplate):
        def afterFlowable(self, flowable):
            if hasattr(flowable,"bookmark_key"):
                ...
                self.canv.addOutlineEntry(flowable.getPlainText(),flowable.bookmark_key,0,False)
        ...
        for block in page["blocks"]:
            ...
            if kind in ("p","h2","small"):
                ...
                for j,entry in enumerate(pages):
                    if j>1: story.append(Paragraph(f'<link href="#p{j}">{entry["title"]}</link> … {j+1}',styles["p"]))
            elif kind=="code":
                style=ParagraphStyle("code",fontName="DV",fontSize=8,leading=11,backColor=colors.HexColor("#F5F7FB"),borderPadding=8,spaceAfter=12)
                ...
                story.append(Paragraph(block[2],styles["small"]));story.append(Spacer(1,7));markdown.append(f"![{block[2]}](figures/{block[1]})\n")
            elif kind=="table":
    ...
    doc=ReportDoc(str(out),pagesize=(595.28,841.89),leftMargin=54,rightMargin=54,
