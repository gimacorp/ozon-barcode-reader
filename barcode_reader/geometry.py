...
from .engineering import depth_of_field, working_distance
...
def project(
    ...
    uv = np.column_stack(
        (
            width_px / 2 + focal_px * local[:, 0] / local[:, 2],
...
def evaluate_end_face(c: dict) -> dict:
    ...
    position = np.array([-distance_x, 0.0, c["box_height_mm"] / 2 + h])
    target = np.array([0.0, 0.0, c["box_height_mm"] / 2])
    ...
    grid = np.array(
        [
            [0.0, y, z]
            for y in np.linspace(-c["box_width_mm"] / 2, c["box_width_mm"] / 2, 17)
            for z in np.linspace(0, c["box_height_mm"], 17)
        ]
    )
    ...
    for phase in np.linspace(-motion_step / 2, motion_step / 2, 21):
        ...
        for shift in (np.arange(count) - (count - 1) / 2) * motion_step + phase:
            ...
            u_y, _ = project(points + [0, 0.01, 0], position, target, focal_px, nx, ny)
            u_z, _ = project(points + [0, 0, 0.01], position, target, focal_px, nx, ny)
            jac = np.stack(((u_y - uv) / 0.01, (u_z - uv) / 0.01), axis=2)
...
def evaluate_label_focus(
    c: dict, aperture: float | None = None, required_frames: int = 1
) -> dict:
    ...
    for angle in np.arange(0, 180, 15):
        ...
        for y in np.linspace(
            -c["box_width_mm"] / 2 + margins[0], c["box_width_mm"] / 2 - margins[0], 9
        ):
    ...
    for phase in np.linspace(-step / 2, step / 2, 21):
        ...
        for shift in (np.arange(count) - (count - 1) / 2) * step + phase:
            uv, d = project(
                (patches + [shift, 0, 0]).reshape(-1, 3), pos, target, fp, nx, ny
            )
            valid = (
                (
                    (d >= near)
    ...
    return {
        ...
        "aperture": ap,
        "labels_per_phase": len(patches),
        "phases": 21,
        "min_whole_label_coverage": worst,
        "geometric_dof_mm": far - near,
