...
def pixels_per_module(
    pixels: int, fov_mm: float, module_mm: float, angle_deg: float = 0.0
) -> float:
...
def motion_blur_px(speed_mm_s: float, exposure_us: float, mm_per_pixel: float) -> float:
...
def depth_of_field(
    focal_mm: float, aperture: float, coc_mm: float, focus_mm: float
) -> tuple[float, float]:
...
def summarize(config: dict) -> dict:
    ...
    area_wd = working_distance(
        c["area_focal_length_mm"], sensor_area, c["area_fov_width_mm"]
    )
    ...
    near, far = depth_of_field(
        ...
        c["area_aperture"],
        c["area_focus_coc_px"] * c["area_sensor_pixel_um"] / 1000,
        area_wd,
    )
    ...
    per_box = (
        c["line_camera_count"] * c["line_pixels"] * lines_per_box * bytes_px
    ...
    last_rear = (
        c["rear_camera_x_mm"]
        + camera_dx
        + c["area_frames_per_box"] / 2 * v / c["area_frame_rate_hz"]
    )
    rounding = positive(
        c["observation_position_rounding_mm"], "observation_position_rounding_mm"
    )
    ...
    return {
        ...
        "line_px_module_vertical": pixels_per_module(
            c["line_pixels"], c["line_fov_vertical_mm"], c["min_module_mm"]
        ),
        ...
        "area_px_module_center": pixels_per_module(
            c["area_pixels_x"], c["area_fov_width_mm"], c["min_module_mm"], angle
        ),
        "area_blur_conservative_px": motion_blur_px(
            v, c["area_exposure_us"], area_step
        ),
        "area_focus_near_mm": near,
        "area_focus_far_mm": far,
        "area_dof_mm": far - near,
        "area_frame_movement_mm": v / c["area_frame_rate_hz"],
        "diffraction_airy_diameter_px_550nm": 2.44
        * 0.00055
        * c["area_aperture"]
        / (c["area_sensor_pixel_um"] / 1000),
        "line_working_distance_horizontal_mm": working_distance(
            c["line_focal_length_mm"],
            c["line_pixels"] * c["line_sensor_pixel_um"] / 1000,
            c["line_fov_horizontal_mm"],
        ),
        "line_working_distance_vertical_mm": working_distance(
            c["line_focal_length_mm"],
            c["line_pixels"] * c["line_sensor_pixel_um"] / 1000,
        ...
        "peak_total_MB_s": (
            c["line_camera_count"] * line_rate_bytes
            + c["area_camera_count"] * area_frame * c["area_frame_rate_hz"]
        )
        / 1e6,
        ...
        "encoder_step_mm": c["encoder_wheel_circumference_mm"]
        / c["encoder_pulses_per_rev"],
        ...
        "overlap_covers_label": c["line_strip_overlap_rows"] * line_step
        >= label_diagonal,
        "zero_errors_n100_upper95": zero_failures_upper_bound(100),
        "trials_for_999_with_zero_failures": math.ceil(
            math.log(0.05) / math.log(0.999)
        ),
    }
