...
def decode_file_bounded(path, timeout_s=5., *, enhanced=False, formats="Code128"):
    ...
    command=[sys.executable,"-m","barcode_reader.worker",str(path),formats,"1" if enhanced else "0"]
    try:
...
def main():
    ...
    try:
        ...
        if image is None:raise ValueError("Не удалось открыть изображение")
