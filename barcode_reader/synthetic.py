...
def label(payload: str, module_px: float = 3.0):
    ...
    return cv2.resize(
        canvas,
        (round((len(bits) + 24) * module_px), round(72 * module_px)),
...
def scene(payloads, rng, level, frame_index):
    ...
    for i, payload in enumerate(payloads):
        ...
        stamp = np.array(
            stamp.rotate(
                angle, resample=Image.Resampling.BICUBIC, expand=True, fillcolor=178
            )
        )
        # Трапеция моделирует наклон плоской этикетки; её положение неизвестно reader.
        h, w = stamp.shape
        a = 0 if level == "clean" else rng.uniform(0, 0.11) * min(h, w)
        src = np.float32([[0, 0], [w - 1, 0], [w - 1, h - 1], [0, h - 1]])
        dst = np.float32([[a, 0], [w - 1 - a, a], [w - 1, h - 1 - a], [0, h - 1]])
        stamp = cv2.warpPerspective(
            stamp, cv2.getPerspectiveTransform(src, dst), (w, h), borderValue=178
        )
        if h > 450 or w > 590:
            ratio = min(450 / h, 590 / w)
            stamp = cv2.resize(
                stamp, None, fx=ratio, fy=ratio, interpolation=cv2.INTER_AREA
            )
            h, w = stamp.shape
        cx, cy = centers[i]
    ...
    if level != "clean":
        ...
        contrast = (
            rng.uniform(0.18, 0.65) if level == "hard" else rng.uniform(0.55, 0.95)
        )
