...
class Assigner:
    def __init__(self) -> None:
        self.assigned: dict[tuple[str, str, str], tuple[str, str]] = {}
    ...
    def assign(
        ...
        if key in self.assigned and self.assigned[key] != matches[0]:
            return None
        self.assigned[key] = matches[0]
        return matches[0]

    def release_boxes(self, box_ids: set[str]) -> int:
        keys = [key for key, owner in self.assigned.items() if owner[0] in box_ids]
        for key in keys:
            del self.assigned[key]
        return len(keys)


def observe_assigned(
    assigner,
    tracker,
    *,
    camera,
    ...
    owner = assigner.assign(
        camera, frame_id, roi_id, detection.polygon, image_to_face, regions
    )
    if owner is None:
        return False
    box_id, face = owner
