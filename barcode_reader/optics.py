...
def diffraction_mtf(frequency_lp_mm, aperture, wavelength_mm=.00055):
...
def disk_mtf(frequency_lp_mm, diameter_mm):
    ...
    term=np.ones_like(x,dtype=float);out=term.copy()
    for k in range(1,40):
        term*=-(x*x/4)/(k*(k+1));out+=term
    return out

def contrast(module_px=3.73, pixel_mm=.0032, aperture=11., coc_px=2., motion_px=.5, aberration_sigma_px=.35):
    f=1/(2*module_px*pixel_mm)
    terms={'diffraction':float(diffraction_mtf(f,aperture)),
           ...
           'aberration_assumption':float(np.exp(-2*np.pi**2*(aberration_sigma_px*pixel_mm*f)**2))}
    return {'frequency_lp_mm':f,'terms':terms,'mtf_product':math.prod(terms.values())}

def defocus_diameter(focal,aperture,focus,distance,pixel_mm):
    v0=focal*focus/(focus-focal);v=focal*np.asarray(distance)/(np.asarray(distance)-focal)
    return np.abs(v0-v)/v*focal/aperture/pixel_mm

def degrade(gray,pixel_mm,aperture,coc_px,motion_px=0.,aberration_sigma_px=.35):
    ...
    """
    pad=32;src=np.pad(gray.astype(float),pad,constant_values=255);h,w=src.shape
    ...
    if coc_px>.1:
        n=max(3,int(np.ceil(coc_px))+2);n+=1-n%2
        axis=(np.arange(n*8)+.5)/8-n/2
        yy,xx=np.meshgrid(axis,axis)
        kernel=((xx*xx+yy*yy)<=(coc_px/2)**2).astype(float).reshape(n,8,n,8).mean((1,3))
        if kernel.sum()==0: kernel[n//2,n//2]=1
        kernel/=kernel.sum()
        impulse=np.zeros_like(src);cy,cx=h//2,w//2
