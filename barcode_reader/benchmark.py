...
def run(output="results", boxes=60, seed=20260909):
    ...
    for i, sample in enumerate(dataset(seed, boxes)):
        ...
        predictions = {}
        for name, frames, enhanced, diagonal in [
            ("one_frame", sample["frames"][1:2], False, False),
        ...
        if i < 3:
            ...
            (examples / f"{sample['box_id']}.json").write_text(
                json.dumps(
    ...
    for method in dict.fromkeys(r["method"] for r in rows):
        for level in ("all", "clean", "moderate", "hard"):
            rs = [
                r
                for r in rows
                if r["method"] == method and (level == "all" or r["level"] == level)
            ]
            if not rs:
                continue
            ...
            metrics.append(
                {
                    ...
                    "code_recall": tp / (tp + fn),
                    "recall_ci95": wilson(tp, tp + fn),
                    "code_precision": tp / (tp + fp) if tp + fp else None,
                    "false_values": fp,
                    "latency_p50_ms": float(
                        np.percentile([r["latency_ms"] for r in rs], 50)
                    ),
                    "latency_p95_ms": float(
                        np.percentile([r["latency_ms"] for r in rs], 95)
                    ),
                }
            )
    result = {
        "metadata": metadata(load_config(), boxes=boxes, seed=seed),
        "seed": seed,
        "boxes": boxes,
        "frames_per_box": 3,
