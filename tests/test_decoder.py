...
from barcode_reader.decoder import decode, decode_strips, parse_formats, strips
...
def test_multiple_symbols():
    ...
    for i, text in enumerate(["BOX000001", "BOX000002"]):
        stamp = label(text, 2)
        canvas[
            30 + 300 * i : 30 + 300 * i + stamp.shape[0], 30 : 30 + stamp.shape[1]
        ] = stamp
...
def test_arbitrary_orientation_and_original_coordinates(angle):
    stamp = np.array(
        Image.fromarray(label("ANGLE000001", 3.86)).rotate(
...
def test_symbol_allowlist_is_explicit():
    ...
    assert (
        decode(label("TEST000001"), formats="Code128,DataMatrix")[0].text
        == "TEST000001"
    )
    with pytest.raises(ValueError):
        parse_formats("Code128,Typo")
