...
def tracker():
    t = BoxTracker()
    t.register(
        "A", 0, 2, 3, contract=CaptureContract.image_demo({f: "frame-1" for f in FACES})
    )
    return t
...
def complete(t, ds=(D,)):
    for face in FACES:
        t.observe("A", face, "frame-1", 1, 1.2, ds if face == "top" else [])
...
def test_exact_deadline_and_closed_result():
    t = tracker()
    complete(t)
    assert t.observe("A", "top", "frame-1", 2, 3, [D])
...
@pytest.mark.parametrize(
    "mode,expected",
    [("empty", "no_read"), ("missing", "incomplete_views"), ("late", "late")],
)
def test_exception_states(mode, expected):
    t = tracker()
    if mode != "missing":
        complete(t, [] if mode == "empty" else [D])
...
def test_finalization_waits_for_completed_decoding():
    t = tracker()
    t.observe("A", "top", "frame-1", 1.9, 2.9, [D])
    with pytest.raises(ValueError):
        t.finalize("A", 2.1)
...
def test_plc_keeps_immutable_receipt_snapshot():
    t = tracker()
    complete(t)
    message = t.finalize("A", 2.5)
    plc = MockPLC()
    plc.receive(message, 2.6)
    message["codes"][0]["text"] = "CHANGED"
    with pytest.raises(ValueError):
        plc.receive(message, 2.7)
    assert plc.receipts[message["message_id"]]["message"]["codes"][0]["text"] == "VALUE"
