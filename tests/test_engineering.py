...
from barcode_reader.engineering import (
    ...
    summarize,
    working_distance,
    zero_failures_upper_bound,
)
from barcode_reader.geometry import evaluate_end_face


def test_sampling_uses_module_not_label():
    assert pixels_per_module(8192, 700, 0.33) == pytest.approx(3.861942857)
    assert pixels_per_module(8192, 700, 0.33, 60) == pytest.approx(1.9309714285)


@pytest.mark.parametrize(
    "fov,module,angle", [(0, 0.33, 0), (700, 0, 0), (700, 0.33, 90)]
)
...
def test_motion_blur_unit_conversion():
    assert motion_blur_px(1000, 40, 1 / 12) == pytest.approx(0.48)


def test_deadline_measures_front_of_box():
    assert deadline_budget(3000, 1400, 600, 1000, 0.1, 0.25) == pytest.approx(0.65)
    assert deadline_budget(2000, 1400, 600, 1000, 0.1, 0.25) < 0


def test_dof_contains_focus():
    near, far = depth_of_field(50, 8, 0.0064, 1300)
    assert near < 1300 < far
    assert far - near > 60


def test_required_evidence_is_not_100_trials():
    assert zero_failures_upper_bound(100) == pytest.approx(0.02951305)
    assert zero_failures_upper_bound(2995) < 0.001
...
def test_speed_changes_last_observation_and_deadline():
    ...
    assert fast["remaining_processing_delivery_s"] == pytest.approx(0.065)


@pytest.mark.parametrize("bad", [float("nan"), float("inf"), -1])
def test_nonfinite_blur_rejected(bad):
    with pytest.raises(ValueError):
        motion_blur_px(bad, 40, 0.08)
    with pytest.raises(ValueError):
        motion_blur_px(1000, bad, 0.08)
